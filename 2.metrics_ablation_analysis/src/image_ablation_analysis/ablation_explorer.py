"""
ablation_explorer.py

Class for visualizing random ablation examples with some level of filtering
    grouping by ablation type and parameter and cropping to center supported.
"""
...
class AblationExplorer:
    def __init__(
        self, 
        abl_index_df: pd.DataFrame | None = None,
        abl_index_path: Path | None = None,
    ):
        """
        Initialize the explorer

        :param abl_index_df: Ablation Index DataFrame produced from 
            `image_ablation_analysis.AblationRunner`, must be provided if 
            abl_index_path is not provided.
        :param abl_index_path: Path to the ablation index file, must be 
            provided if abl_index_df is not provided.
        """
        if abl_index_df is not None:
            self._df = abl_index_df
        elif abl_index_path is not None:
            abl_indices = list((abl_index_path / "ablated_index").glob("aug_index_*.parquet"))
            self._df = pd.concat([pd.read_parquet(f) for f in abl_indices]).reset_index(drop=True)
        else:
            raise ValueError("Either abl_index_df or abl_index_path must be provided.")

        self.meta = self._build_meta(self._df)

    @staticmethod
    def _to_list(x):
        """
        Helper coercion function to parse out the list-like representation
            of the param_swept and param_values columns in the meta DataFrame.
        """
    ...
    def _first_or_nan(cls, x):
        """
        Helper function to extract the first element from a list-like column, 
            or return NaN if empty.
        """
        vals = cls._to_list(x)
    ...
    def _build_meta(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Helper function to build the DataFrame used for grouping/filtering in the explorer.
        """
        meta = df.copy()

        if not "ablation_type" in meta.columns:
            meta["ablation_type"] = meta["config_id"].apply(lambda x: x.split(":")[1])
    ...
    def enumerate_combinations(self) -> pd.DataFrame:
        """
        Enumerate all unique combinations of ablation types and swept parameters.
        """
        return (
    ...
    def _candidate_groups(
        ...
        if additional_filter:
            for col, val in additional_filter.items():
                m = m[m[col] == val]
                if m.empty:
                    raise ValueError(f"No candidates found after applying additional filter: {col} == {val}")
    ...
    def plot_random_combo(
        ...
        Main function to plot a random combo of original + augmented images for
            a given ablation type and parameter, with additional simple filtering
            and display options allowed

        :param ablation_type: The ablation type to filter by (e.g. "gaussian_noise").
        :param param_swept: The parameter swept to filter by (e.g. "stddev").
        :param seed: Optional random seed for reproducibility of the random combo chosen.
        :param sample_n_levels: Optional number of levels to sample for the parameter swept, if there are many. If None, all levels will be plotted.
        :param grayscale_limits_from: Whether to compute grayscale limits for display from the original image only, or from all images in the combo. Must be one of "original" or "all".
        :param crop_center: Optional integer size to crop the center of the images to (e.g. 256 would crop to a 256x256 square centered on the image). If None, no cropping is done.
        :param additional_filter: Optional dict of additional column filters to apply when selecting candidate groups, in the form
        """
